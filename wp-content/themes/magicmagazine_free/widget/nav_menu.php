...
class nav_menu extends WP_Widget {
	...
	function nav_menu()
	{
		...
		parent::WP_Widget($id_base="nav_menu",$name='自定义菜单，发送一个菜单到界面上篇【侧边栏独占】',$widget_ops,$control_ops);  
...
<p>   
   ...
   <label for="<?php echo $this->get_field_id('nav_menu_m'); ?>">选择显示模式</label>
			<select id="<?php echo $this->get_field_id('nav_menu_m'); ?>" name="<?php echo $this->get_field_name('nav_menu_m'); ?>">
				<option value="">默认一行2列并排</option>
		         <option value="1"<?php if($nav_menu_m){echo'selected="selected"'; } ?> >一行一个</option>
			</select>
	...
	$nav_menu_m =apply_filters('widget_title', empty($instance['nav_menu_m']) ? __('') : $instance['nav_menu_m']);
...
<div class="nav_menu_widgetss box"<?php if($nav_menu_m){echo'id="onepices"'; }?> >
