  ...
  <div class="xiaot">
    ...
    <div class="xiaot">
                    ...
                    <label  for="left_right ">调换位置:</label>
           ...
           <b class="bt">内页顶部以及底部图片文字设定</b>
            ...
            <label  for="top_pic_move">【手机端的顶部图片】(尺寸：700*158)</label> 
              <input type="text" size="60"  name="top_pic_move" id="top_pic_move" value="" readonly="true"/>   
               <br /> 
              <p class="tishiwenzi">当前使用的主题为免费版，付费版可以解锁兼容手机移动端功能<a target="_blank" href="http://www.themepark.com.cn/msmhwordpressqyzt.html"> 查看付费版详情</a></p>
            </div>
            ...
            <p>显示文章数量自定义，在设定分类目录的列表页现实的数量时，记得要把默认的WordPress数量设为1，否则会出现404错误，设置方法请在 “设置” -- “阅读”中 ，将现实的文章数量设为1，即可。</p>
              <p> <label  for="list_nmber1">纯文字列表模板：</label> 
                <input type="text" size="20"  name="list_nmber1" id="list_nmber1"readonly="true" /> 
   ...
   <p class="tishiwenzi">当前使用的主题为免费版，付费版可以解锁分类目录、页面、文章多模板选择功能<a target="_blank" href="http://www.themepark.com.cn/msmhwordpressqyzt.html"> 查看付费版详情</a></p>
               <p><?php $pic_size_singln= get_option('mytheme_pic_size_singln');  ?>  
 <label  for="pic_size_singln">内页中推荐的相关推荐图片尺寸:</label>
                  <select name="pic_size_singln" id="pic_size_singln">
                   <option value=''<?php if ( $pic_size_singln ==="" ) {echo "selected='selected'";}?>>横向长方形</option>
                    <option value='case_full'<?php if ( $pic_size_singln ==="case_full" ) {echo "selected='selected'";}?>>竖向长方形</option>
                   <option value='fang'<?php if ( $pic_size_singln ==="fang" ) {echo "selected='selected'";}?>>正方形</option>
	             </select> <br /></p>
            ...
            <p class="tishiwenzi">当前使用的主题为免费版，免费版只有两种尺寸的略缩图产生<a target="_blank" href="http://www.themepark.com.cn/msmhwordpressqyzt.html"> 查看付费版详情</a></p> 
    <?php if(get_option('mytheme_case_thumbnails')){$case_thumbnails=get_option('mytheme_case_thumbnails');}else{$case_thumbnails= get_bloginfo('template_url').'/images/thumbnails/case.jpg';} 
		  if(get_option('mytheme_fang_thumbnails')){$fang_thumbnails=get_option('mytheme_fang_thumbnails');}else{$fang_thumbnails= get_bloginfo('template_url').'/images/thumbnails/fang.jpg';}
		  if(get_option('mytheme_four_s_thumbnails')){$four_s_thumbnails=get_option('mytheme_four_s_thumbnails');}else{$four_s_thumbnails= get_bloginfo('template_url').'/images/thumbnails/four_s.jpg';}
		  if(get_option('mytheme_news_thumbnails')){$news_thumbnails=get_option('mytheme_news_thumbnails');}else{$news_thumbnails= get_bloginfo('template_url').'/images/thumbnails/news.jpg';}
		  if(get_option('mytheme_case_full_thumbnails')){$case_full_thumbnails=get_option('mytheme_case_full_thumbnails');}else{$case_full_thumbnails= get_bloginfo('template_url').'/images/thumbnails/case_full.jpg';}
            ...
            <label  for="case_thumbnails">图片：287, 287</label> 
             ...
             <div >  <img  style="width:100px; height:auto;"src="<?php echo $four_s_thumbnails; ?>"/><br />
            <label  for="four_s_thumbnails">图片：480,574</label> 
